 ...
 * this site helps immensely whith durchblicking post processing volume related stuff: https://discussions.unity.com/t/how-to-modify-post-processing-profiles-in-script/761287/3
 * Also: Source of the colorLut Field for the profile: https://docs.unity3d.com/Packages/com.unity.postprocessing@2.1/api/UnityEngine.Rendering.PostProcessing.ColorGrading.html#UnityEngine_Rendering_PostProcessing_ColorGrading_ldrLut
 */
using System;
...
public class MachadoSim : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //passthrough.TryGetComponent(out OVRPassthroughLayer passthroughLayer);
        if (passthrough != null)
        {
    ...
    public void CreatePersonalizedLUT(UserDataCVD user)
    {
        ...
        MySimMatrix = finalMatrix.GetMatrix(); //Only for debugging to see values in Inspector
        CreateLUT(finalMatrix.GetMatrix(), user);
    }

    private void DeletePersonalizedLUT(string name)
    {
    ...
    public void LoadPersonalizedLUT(string name)
    {
        ...
        if (name == null)
        {
            pathFilename = lutLoadPath + "/CVD_LUT_DefaultProfile";
        }

        else
        {
            pathFilename = lutLoadPath + "/CVD_LUT_" + name;
        }
        var lutTex = Resources.Load<Texture2D>(pathFilename);

        Debug.LogWarning("Wir laden gerade das Profil: " + name);
        Debug.LogWarning("Das ergibt den File Loading Path: " + pathFilename);
        Debug.LogWarning("Existiert in der entsprechenden Variable eigentlich ne Textur?: " + CheckAssigned(lutTex));

        ApplyLUTPostProcessing(lutTex);
        ApplyLUTPassthrough(lutTex);
    }

    private bool CheckAssigned(object myObj)
    {
        if(myObj != null)
        {
            return true;
    ...
    private void ApplyLUTPostProcessing(Texture2D tex)
    {
        var postprocessing = GameObject.Find("Post Processing Volume");
        if (postprocessing != null)
        {
            //ppVolume = postprocessing.GetComponent<PostProcessVolume>();

            //Mit Maus über Profile hovern für mehr Infos.
            //Es könnte sinnvoll sein, das Profil direkt in den Assets zu bearbeiten und die Referenz zu diesem Asset über alle
            //Szenen hinweg zu behalten

            //var profile = ppVolume.profile;
            //var lut = profile.GetSetting<ColorGrading>().ldrLut;
            //lut.Override(tex);

            //GameObject.Find("Post Processing Volume").GetComponent<PostProcessVolume>()
            //.profile.GetSetting<ColorGrading>().ldrLut.Override(tex);
            //Debug.Log("Irgendwo wurde anscheinend eine Post Processing Volume angefasst...");

            /*
            //Dashier funktioniert! Aber ich versuche unten nochmal was weniger Fehleranfälliges
            var volume = GameObject.Find("Post Processing Volume").GetComponent<PostProcessVolume>();
            if (volume.profile.TryGetSettings(out ColorGrading colorGrading))
            {
                Debug.Log("Color Grading gefunden!");
            }
            else
            {
                Debug.LogWarning("Kein ColorGrading im PostProcessProfile gefunden!");
            }

            if (colorGrading != null && tex != null)
            {
                colorGrading.ldrLut.value = tex;
                colorGrading.ldrLut.overrideState = true;
                Debug.Log("Neue LUT gesetzt!");
            }
            */

            //Ich versuche, ein bestehendes Profil zu verändern und auf die Volume in der Szene anzuwenden ohne Overrides.
            var volume = GameObject.Find("Post Processing Volume").GetComponent<PostProcessVolume>();

            string ppProfilePath = "Assets/Resources/PP_Profiles/Personalized_LUT_PP.asset";
            var profile = Resources.Load<PostProcessProfile>(ppProfilePath);

            if (profile != null && tex != null)
            {
    ...
    private void ApplyLUTPassthrough(Texture2D tex)
    {
        if (m_passthroughLayer != null)
        {
    ...
    private void CreateLUT(Matrix4x4 matrix, UserDataCVD myUser)
    {
        const int resolution = 32; //auf 32 setzen für volle 255 value Resolution
        var lutColors = ColorArrayFromMatrix(matrix, resolution);
        var lutTex = CreateLutAsset(lutColors, resolution);
        SaveTextureAsPNG(lutTex, myUser);
    }
    ...
    void CreateLUTFromMatrix(Matrix4x4 matrix)
    {
        ...
        Debug.Log("CVDMatrix wurde in Farben konvertiert.");
    ...
    private Color[] ColorArrayFromMatrix(Matrix4x4 matrix, int resolution)
    {
        Color[] lutColors = new Color[resolution * resolution * resolution];
        //Matrix4x4 matrix = DeficiencyMatrix.GetMatrix();
    ...
    private Texture2D CreateLutAsset(Color[] colors, int res)
    {
    ...
    private void SaveTextureAsPNG(Texture2D texture, UserDataCVD myUser)
    {
        byte[] bytes = texture.EncodeToPNG();
        if (!System.IO.Directory.Exists(lutPath))
        {
            System.IO.Directory.CreateDirectory(lutPath);
        }
        System.IO.File.WriteAllBytes(lutPath + "/CVD_LUT_" + myUser.Name + ".png", bytes); //Hier bald durch Profilnamen individualisieren!
        Debug.Log(bytes.Length / 1024 + "Kb was saved as: " + lutPath);
...
#region Backup wo die LUT noch mit der Generierung gespeichert wurde
/*
public class MachadoSim : MonoBehaviour
{
    private OVRPassthroughLayer m_passthroughLayer;
...
*/
#endregion Backup wo die LUT noch mit der Generierung gespeichert wurde